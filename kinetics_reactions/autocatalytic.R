autocatalytic = function(a = 139, b = 1, x = 0, cycles = 1000, probability = 100){
  ...
  for (i in 2:(cycles + 1)){
    ...
    if(jar[draw_marble[1]] == "A" & jar[draw_marble[2]] == "B" 
       & pick_prob <= probability){
      jar[draw_marble[1]] = "B" 
    } else {
    if(jar[draw_marble[1]] == "B" & jar[draw_marble[2]] == "A" 
       & pick_prob <= probability){
      jar[draw_marble[2]] = "B" 
    } 
    }
  ...
  out = list("mechanism" = "autocatalytic",
