bimolecular = function(a = 140, b = 0, x = 0, cycles = 500, probability = 100){
  ...
  # evalute and change As to Bs or leave as is
  # update counts for A, B, and X
  for (i in 2:(cycles + 1)){
    draw_marble = sample(id_marble, 2)
    pick_prob = sample(id_prob, 1)
    if(jar[draw_marble[1]] == "A" & jar[draw_marble[2]] == "A" 
  ...
  out = list("mechanism" = "bimolecular",
