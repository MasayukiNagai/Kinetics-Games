consecutive = function(a = 140, b = 0, x = 0, cycles = 600, probability = 100){
  ...
  # draw marble; probability for X to B only
  # evalute and change A to X, change X to B, or leave as is
  # update counts for A, B, and X
  for (i in 2:(cycles + 1)){
    draw_marble = sample(id_marble, 1)
    pick_prob = sample(id_prob, 1)
    if(jar[draw_marble] == "A"){
      jar[draw_marble] = "X" 
    } else {
    if(jar[draw_marble] == "X" & pick_prob <= probability){
      jar[draw_marble] = "B"
    }
      }
  ...
  out = list("mechanism" = "consecutive",
